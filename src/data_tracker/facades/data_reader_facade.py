...
class DataReaderFacade:
    ...
    # def __init__(self, search_paths=[os.path.join(os.path.abspath('..'), 'data_readers')], path_user_scripts=None):
        ...
        self.reader_classes = self.find_reader_classes(extra_folder=self.path_user_scripts)
        # print('\nIIIIIIIIIIIIIIIIIIIII\n\n\n\n\n\n\n\n\n\n\n')
        # print(reader_classes)
        # for reader in reader_classes:
        #     print(reader.__name__)
        # self.reader_classes = reader_classes
        self._import_and_register(self.reader_classes)
        # self._discover_readers()

    def find_reader_classes(self, extra_folder=None):
        reader_classes = []

        # === 1. Built-in readers from package ===
        for _, module_name, is_pkg in pkgutil.iter_modules(readers_pkg.__path__, readers_pkg.__name__ + "."):
            ...
            try:
                ...
                print("Error importing package module:", e)
                continue

            reader_classes += self._find_subclasses_in_module(module)

        # === 2. Custom readers from external path ===
        extra_folders = extra_folder if isinstance(extra_folder, list) else [extra_folder]
        for extra_folder in extra_folders:

            folder = Path(extra_folder).resolve()
            if folder.exists() and folder.is_dir():
                for file_path in folder.glob("*.py"):
                    module_name = f"user_script_{file_path.stem}"
                    ...
                    try:
                        ...
                        print(f"Error importing user script {file_path.name}:", e)

        return reader_classes


    def _find_subclasses_in_module(self, module):
        return [
            obj
            for name, obj in inspect.getmembers(module, inspect.isclass)
            if issubclass(obj, DataReaderStrategy)
            and obj is not DataReaderStrategy
    ...
    def _import_and_register(self, reader_classes):

        for obj in reader_classes:#inspect.getmembers(module, inspect.isclass):
    ...
    # def __import_and_register(self, module_name, file_path):
