"""
build command: "PYO3_CONFIG_FILE=C:\\Users\\pkv190\\AppData\\Local\\Temp\\pyoxidizerK9dQGk\\artifacts\\pyo3-build-config-file.txt" "PYOXIDIZER_ARTIFACT_DIR=C:\\Users\\pkv190\\AppData\\Local\\Temp\\pyoxidizerK9dQGk\\artifacts" 'PYOXIDIZER_REUSE_ARTIFACTS=1' "RUSTC=C:\\Users\\pkv190\\AppData\\Local\\pyoxidizer\\rust\\1.66.0-x86_64-pc-windows-msvc\\bin\\rustc.exe" "C:\\Users\\pkv190\\AppData\\Local\\pyoxidizer\\rust\\1.66.0-x86_64-pc-windows-msvc\\bin\\cargo.exe" build --target x86_64-pc-windows-msvc --target-dir "C:\\Users\\pkv190\\AppData\\Local\\Temp\\pyoxidizerK9dQGk\\build\\target" --bin data_tracker_app --locked --no-default-features --features build-mode-prebuilt-artifacts
"""
...
def make_exe():
    ...
    # python_config.run_command = "print(123); import data_tracker; print(123)"
    ...
    # python_config.run_filename = "./data_tracker_main.py"
    python_config.run_filename = "./test.py"

    # Produce a PythonExecutable from a Python distribution, embedded
    # resources, and other options. The returned object represents the
    # standalone executable that will be built.
    exe = dist.to_python_executable(
        name="data_tracker_app",
    ...
    # https://stackoverflow.com/questions/69770716/extension-module-cannot-be-loaded-from-memory-but-memory-loading-required
    # https://github.com/indygreg/PyOxidizer/issues/339#issuecomment-1014632560
    ...
    # for resource in exe.pip_download(["numpy", "pandas", "matplotlib"]):
    ...
    # Correct general structure for in-memory local package:
    for resource in exe.read_package_root(
        path="../src/",      # Path containing the package directory
        packages=["data_tracker"], # List of package names to find
    ):
        # print(f"Found resource for data_tracker: {resource.package_path}") # Debug print
        resource.add_location = "in-memory" # Crucial for embedding in memory
        exe.add_python_resource(resource)
